...
import { CloseIcon, TrashIcon, BagIcon } from "../Home components/Icons.jsx";

export default function CartDrawer() {
  const { items, removeFromCart, updateQty, totalPrice, isOpen, setIsOpen } =
    useCart();

  return (
    <>
      {/* پس‌زمینه تیره - فقط موبایل */}
      <div
      ...
      {/* کلیک بیرون برای بستن - فقط دسکتاپ */}
      {isOpen && (
        <div
          onClick={() => setIsOpen(false)}
      ...
      <aside
        className={`fixed z-50 bg-white flex flex-col
          top-0 right-0 h-full w-full sm:w-96 transition-transform duration-300
          ${isOpen ? "translate-x-0" : "translate-x-full"}
          md:top-36 md:right-8 md:h-auto md:max-h-[75vh] md:w-96 md:rounded-2xl
          md:shadow-2xl md:border md:border-neutral-100 md:translate-x-0
          md:origin-top-right md:transition-all md:duration-200
          ${
            isOpen
